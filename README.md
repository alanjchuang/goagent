...
- `执行 Applications`：自动列出 `applications/**/workflows/*.yaml`，选择应用、填写任务并运行；支持同时创建多个执行，每次执行都有独立的历史结果。
...
1. 在 `和模型对话` 页签输入问题并发送。
2. 在 `执行 Applications` 页签选择一个应用，填写任务并点击运行；可以连续创建多个执行，结果会在历史列表中隔离展示。
...
- `执行 Applications` / run applications: list `applications/**/workflows/*.yaml`, choose one, enter a task, and run it. Multiple runs can execute concurrently, and each run has an isolated history result.
...
2. Choose and run an application in the `执行 Applications` tab. You can create multiple runs, and results are isolated in the run history.
